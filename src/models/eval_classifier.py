...
MODEL_PATH = PROJECT_ROOT / "models" / "root_cause_classifier" / "best_model.pt"
...
def main():
    ...
    for t in [0.25, 0.3, 0.35, 0.4, 0.45, 0.5]:
        preds, lbls = evaluate(model, val_loader, device, threshold=t)
    ...
    # Test evaluation
    test_preds, test_labels = evaluate(model, test_loader, device, threshold=best_t)
